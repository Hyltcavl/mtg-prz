...
def get_amount_of_pages(cmc: int):
    ...
    rsp = requests.get(f'https://astraeus.dragonslair.se/product/magic/card-singles/store:kungsholmstorg/cmc-{cmc}/1')
    soup = BeautifulSoup(rsp.text, 'html.parser')
    pages_count = 1 #default value if no pages are found
    try:
        a_element = soup.find(class_='container align-center pagination').find("ul").find_all("li").pop().a
...
def getCard(tr):
    ...
    if bool(re.search(r'\(Skadad\)', name, re.IGNORECASE)) or bool(re.search(r'\(Spelad\)', name, re.IGNORECASE)) or bool(re.search(r'\( Skadad \)', name, re.IGNORECASE)) or bool(re.search(r'\[Token\]', name, re.IGNORECASE)):
        return {}

    foil = bool(re.search(r'\(Foil\)', name, re.IGNORECASE)) or bool(re.search(r'\(Etched Foil\)', name, re.IGNORECASE)) or bool(re.search(r'\(Foil Etched\)', name, re.IGNORECASE))
