...
files = [file for file in os.listdir(alphaspel_folder) if file.startswith(cards_file_prefix)]
...
cards_file_prefix = f'cards_{today_date_as_string}'
files = [file for file in os.listdir(dragonslair_folder) if file.startswith(cards_file_prefix)]
...
prices_of_cards_alpha = compare_prices(alphaspel_cards_list, scryfall_card_list, "alphaspel")
prices_of_cards_dl = compare_prices(dragonslair_cards_list, scryfall_card_list, "alphaspel")

print_to_new_file("results", f'alpha_prices_{start_time_as_string}.json', json.dumps(prices_of_cards_alpha))
print_to_new_file("results", f'dl_prices_{start_time_as_string}.json', json.dumps(prices_of_cards_dl))
...
print_to_new_file("nice_prices", f'alpha_cards_{today_date_as_string}.json', json.dumps(nice_prices_alpha))
print_to_new_file("nice_prices", f'dl_cards_{today_date_as_string}.json', json.dumps(nice_prices_dl))

difference_in_minutes =get_time_difference_in_minutes(start_time)
print(f'Comparing prices started {start_time_as_string} and took {difference_in_minutes}')
